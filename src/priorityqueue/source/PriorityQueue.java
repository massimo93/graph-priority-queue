...
public class PriorityQueue<T, P> {
   ...
   *
   * @param comparator a comparator that implements the priority precedence relation 
   * between the heap structure elements
   */
  ...
  /**
   * Custom constructor to build a heap from a given array and its elements priorities; it's a O(n)
   * implementation
   *
   * @param array the data structure that will fill the priority queue inner heap structure
  ...
  /**
   * An iterative implementation of a method that inserts an element into a priority queue
   *
   ...
   * that needs to be inserted is already in queue
   */
  public void insert(T element, P priority) {
    ...
    while (i > 0 && c.compare(heap.get(parent(i)).prior, heap.get(i).prior) < 0) {
      /*invariant: for each x in i..heap.size()-1 : heap.get(x).prior <= heap.get(right(x)).prior 
                              AND heap.get(x).prior <= heap.get(left(x)).prior -> minHeap*/
      /*invariant: for each x in i..heap.size()-1 : heap.get(x).prior >= heap.get(right(x)).prior 
                              AND heap.get(x).prior >= heap.get(left(x)).prior -> maxHeap*/
      swap(i, parent(i));
      i = parent(i);
    }
  }
  ...
  /**
   * A recursive implementation of a method inserting an element into a heap structure
   * 
  ...
  /**
   * The actual recursive implementation of a method that inserts an element
  ...
  /**
   * An iterative implementation of a method that extracts the first element
  ...
  public T extract() {
    ...
    while (c.compare(heap.get(i).prior, heap.get(left(i)).prior) < 0
      ...
      /*invariant: for each x in 0..i : heap.get(x).prior >= heap.get(parent(x)).prior -> minHeap*/
      /*invariant: for each x in 0..i : heap.get(x).prior <= heap.get(parent(x)).prior -> maxHeap*/
  ...
  /**
   * A recursive implementation of a method extracting the first element from
   * the priority queue
   *
   * @return the removed element if the queue is not empty, null if it is
   * @throws UnsupportedOperationException when calling it on an empty queue
  ...
  /**
   * Method that updates the priority of a given element with a new priority,
   * also passed as param
   *
  ...
  /**
   * Method set to calculate the maximum integer value of a set of values,
   * passed as params
   *
  ...
  /**
   * Auxiliary method getting the heap structure,
  ...
  /**
   * Method telling if the inner heap structure of the priority queue is empty
   * or not
   *
  ...
  /**
   * Method telling if an element, passed as param, is contained in the priority queue
   * 
  ...
  /**
   * Inner class representing the a generic type of elements accepted by the heap structure
  ...
  protected class QueueElement {
     ...
     * 
     * @param element a generic element that can be inserted in the priority queue heap structure
     * @param priority a generic priority value of the corresponding element, passed as param
     */
    ...
    /**
     * Auxiliary method getting the element value of the queue element, 
     * useful for unit testing purposes
     * 
    ...
    /**
     * Auxiliary method getting the priority value of the queue element, 
     * useful for unit testing purposes
     * 
